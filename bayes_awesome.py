...
class SimpleSampler(object):
    ...
    def get_prob(self, query_vals, num_samples):
        ...
        while i < num_samples:  
            satisfies = True
            sampleDict = self.generate_sample()
            for q in query_vals:
                if query_vals.get(q) != sampleDict.get(q):
                    satisfies = False
            if(satisfies):
                trueCounter+=1
            i+=1
        return trueCounter/num_samples  # Fix this line!
...
class RejectionSampler(SimpleSampler):
    ...
    def get_prob(self, query_vals, evidence_vals, num_samples):
        ...
        while num_samples>0:           
            satisfies = True
            satisfiesEvidence = True
            sampleDict = self.generate_sample()
            for e in evidence_vals:
                if evidence_vals.get(e) != sampleDict.get(e):
        ...
        if(keptSamples==0):
            return None
        return trueSamples/keptSamples  # Fix this line!
...
class LikelihoodWeightingSampler(SimpleSampler):
    ...
    def get_prob(self, query_vals, evidence_vals, num_samples):
        ...
        while i>0:
            satisfies = True
            sample_dict, weight = self.generate_sample(evidence_vals)
            for q in query_vals:
                if query_vals.get(q)!=sample_dict.get(q):
                    satisfies = False
            if(satisfies):
                trueWeight+=weight
            totalWeight+=weight
...
def bayes_sample_size_plot(sampler1, sampler2, query, evidence, label1, label2, title, fname):
    ...
    i = 50
    while i<= 10000:
        yValues1.append(sampler1.get_prob(query,evidence,i))
        yValues2.append(sampler2.get_prob(query,evidence,i))
        xValues.append(i)
        i+=20
    two_line_plot(xValues,yValues1,label1,xValues,yValues2,label2,title,fname)
    return
